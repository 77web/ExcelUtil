...
class BookTest extends \PHPUnit_Framework_TestCase
{
    public function test_makeSheetMap()
    {
        ...
        $this->assertEquals('表紙', $map[4]);
    }

    public function test_makeSheetFileMap()
    {
        $zip = $this->makeZipMock(['xl/workbook.xml', 'xl/_rels/workbook.xml.rels']);

        $bookUtil = new Book();
        $map = $bookUtil->makeSheetFileMap($zip);
    ...
    {
        $zip = $this->getMock('Nanaweb\ExcelUtil\ZipArchive');
        $zip
    ...
    {
        return file_get_contents(__DIR__.'/xml/'.$path);
    }
}
